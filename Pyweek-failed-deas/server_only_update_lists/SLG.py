...
class Server(net.Server):
    ...
    def leaveGame(self, avatar):
        if avatar.game:
            avatar.game.player_leave(avatar)
            self.remoteAll('userJoinedLobby')
            self.getGameList(avatar)
            self.getLobbyUsersList(avatar)
    ...
    def join(self, avatar):
        ...
        self.remoteAll('userJoinedLobby', avatar.name)
        self.getGameList(avatar)
        self.getLobbyUsersList(avatar)
    ...
    def leave(self, avatar):
        ...
        if avatar.game:
            ...
            self.remoteAll('userLeftLobby', avatar.name)
    ...
    def update_game_settings(self, game):
        data = (game.game_id, game.name, game.scenario,
                game.get_master().name,
                len(game.players), game.max_players,
                game.playing)
        self.remoteAll('updateGameSettings', data)

    def gameClose(self, game):
        data = (game.game_id, game.name, game.scenario,
                game.last_master.name,
                len(game.players), game.max_players,
                game.playing)
        self.remoteAll('gameClose', data)
...
class Client(net.Client):
    ...
    def remote_userLeftLobby(self, user):
        pass
    def remote_updateGameSettings(self, data):
        pass
    def remote_gameClose(self, data):
        pass

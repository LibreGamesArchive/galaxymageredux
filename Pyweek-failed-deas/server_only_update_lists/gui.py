...
class List(Container):
    ...
    def del_entry(self, opt):
        ent = self.entry_map[opt]
        del self.entry_map[opt]
        l = self.widgets.index(ent)
        self.widgets.remove(ent)

        print l

        if l == 0:
            self.widgets[l].pos = AbsolutePos(self.padding)
        else:
            self.widgets[l].pos.to = self.widgets[l-1]
...
class ViewGameRooms(Container):
    ...
    def del_option(self, gid):
        ent = self.entry_maps[gid]
        del self.entry_map[gid]
        l = self.widgets.index(ent)
        self.widgets.remove(ent)

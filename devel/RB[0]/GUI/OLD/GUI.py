...
class PYTHON_GUI:
        ...
        if self.moving_window:
            ...
            for foo in self.windows:
                ...
                if((foo.rect.collidepoint(x,y)==True)and(foo.visible==True)):
                    ...
                    if foo.moving_slider:
                        bar = foo.moving_slider
                        if bar.active:
                            x_off=x-foo.rect.x
                            y_off=y-foo.rect.y
                            x_widget=x_off-bar.rect.x
                            y_widget=y_off-bar.rect.y
                            bar.callbacks.mouse_ldown(bar, x_widget, y_widget, event)
                        else:
                            foo.moving_slider = None
                        if not pygame.mouse.get_pressed()[0]:
                            foo.moving_slider = None
                    else:
    ...
    def check_inputs(self,events):
        ...
        mouse_pass = self.quick_mouse_test()
        for event in events:
            ...
            if(event.type==pygame.QUIT):
                #send this back to the game to handle
                unused.append(event)
...
class Slider:
    ...
    def slider_mouse_ldown(self,handle,xpos,ypos,event):

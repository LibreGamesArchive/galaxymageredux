...
def image(name=None, filename="None.png"):
    ...
    try:
        ...
        all_images[name] = gfx.load_image(filename)
    except: #this image is in the main area I guess...
        filename = os.path.join("..", "data", "images" ,os.path.split(filename)[1])
        all_images[name] = gfx.load_image(filename)
    return None
...
def map_tile(x=0, y=0, bottom=0, height=5,
    ...
    new = gfx.Tile((terrain.image_top,)+terrain.image_sides,
                         (x, y),
